...
namespace XRCore.Agents
{
    public sealed class XRGuideAgent : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            BootstrapInitialTaskInstruction();
        }

        private void Start()
        {
            // Safety net for Unity execution order:
            // if TaskRunner started before this agent subscribed, bootstrap now.
            BootstrapInitialTaskInstruction();
        }
        ...
        private void BootstrapInitialTaskInstruction()
        {
            ...
            var taskRunner = FindFirstObjectByType<XRTaskRunner>();
            if (taskRunner == null || taskRunner.TaskDefinition == null)
            {
                return;
            }

            if (!taskRunner.IsRunning || taskRunner.CurrentStep == null)
            {
                return;
            }

            _context.Apply(new XRTaskStartedEvent(taskRunner.TaskDefinition));
            _context.Apply(new XRTaskStepChangedEvent(taskRunner.TaskDefinition, taskRunner.CurrentStepIndex, taskRunner.CurrentStep));
            EnqueueTrigger(TriggerTaskStepChanged, critical: true);
        }
